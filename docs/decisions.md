...
No macOS, o material usa `NSGlassEffectView` público no estilo `Regular`, como content view da forma. O adaptador nunca guarda ponteiro Objective-C cru: obtém a view pela hierarquia da janela, restaura o content view ao cair no fallback sólido e atualiza o modo efetivo quando o frontend percebe mudança de transparência, movimento ou esquema de cor. A leitura do raio ocorre no main thread sob o mutex imediatamente antes de instalar o efeito. A redução de transparência força sólido. O preenchimento CSS preserva borda, tipografia clara/escura e uma rail direita visível no painel aberto. O estado vazio é intencional e não inventa repositórios, arquivos ou diffs.
...
A validação nativa limitada no display primário confirmou cinco ciclos pelos controles, `Esc` em uma verificação separada, a geometria 16 × 96 ↔ 960 × 600 ancorada na borda direita e material com conteúdo de outro aplicativo visivelmente desfocado. Após a correção final de sincronização do material, o novo bundle abriu, fixou, recolheu por `Esc` e pelo controle, e abriu novamente. Ainda pendem aceite visual, foco entre aplicativos, clique nas bordas arredondadas e em outro aplicativo, hover/reversão com cursor real, seleção e captura real de ponteiro, interrupção de animação, monitores mistos/desconexão e preferências de acessibilidade. Os testes automatizados comprovam contratos e geometria, não esses cenários.

Durante o morph entre os raios 8 e 20, o polling usa provisoriamente o maior raio. Isso evita reter um clique onde o vidro pode já estar transparente, mas pode também ignorar um canto ainda visível no começo da abertura. A equivalência entre a camada apresentada pelo AppKit e o hit-testing não foi comprovada; não há alegação de click-through para outro aplicativo.
